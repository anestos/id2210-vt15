...
public class NatEmulatorComp extends ComponentDefinition {
    ...
    private Handler<Msg> handleIncomingMsg = new Handler<Msg>() {
        ...
        public void handle(Msg msg) {
            ...
            NatedAddress natedDestination = (NatedAddress) msg.getHeader().getDestination();
            log.trace("{} received msg from:{}", new Object[]{selfAddress.getId(), msg.getHeader().getSource()});
            if (!allowMsg(natedSrc, natedDestination)) {
                log.info("{} dropping msg:{} from:{}. my parents{}", new Object[]{selfAddress, msg, natedSrc, selfAddress.getParents()});
                return;
            }
    ...
    private boolean allowMsg(NatedAddress src, NatedAddress natedDestination) {
        if(selfAddress.isOpen()) {
            return true;
        }
        return selfAddress.getParents().contains(src) || natedDestination.getParents().contains(src);
    }
